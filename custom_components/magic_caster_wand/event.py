"""Support for mcw event entities."""
...
class McwEventEntity(EventEntity):
    """Representation of a Mcw event entity."""
    ...
    def __init__(
        ...
        address: str,
        event_class: str,
        event: McwBleEvent | None,
    ) -> None:
        """Initialise a Mcw event entity."""
    ...
    @callback
    def _async_handle_event(self, event: McwBleEvent) -> None:
        self._trigger_event(event[EVENT_TYPE], event[EVENT_PROPERTIES])
...
async def async_setup_entry(
    ...
    async_add_entities(
        # Matches logic in PassiveBluetoothProcessorEntity
        McwEventEntity(address_event_class[0], address_event_class[2], None)
    ...
    @callback
    def _async_discovered_event_class(event_class: str, event: McwBleEvent) -> None:
        """Handle a newly discovered event class with or without a postfix."""
        async_add_entities([McwEventEntity(address, event_class, event)])
